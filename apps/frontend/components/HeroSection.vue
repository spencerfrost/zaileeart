<template>
  <section class="bg-dutch-white rounded-3xl p-8 mb-12 relative overflow-hidden">
    ...
    <div v-else-if="artistProfile" class="max-w-2xl">
      <h1 v-if="artistProfile.name" class="text-4xl font-bold text-rich-black mb-4">Welcome to {{ artistProfile.name }}'s Artistic World</h1>
      <p v-if="artistProfile.title || artistProfile.pronouns" class="text-l text-rich-black mb-6 italic">
      ...
      <p v-if="artistProfile.short_bio" class="text-l text-rich-black mb-6">{{ artistProfile.short_bio }}</p>
      <NuxtLink to="/gallery" class="bg-air-blue text-white px-6 py-2 rounded-full hover:bg-air-blue transition-colors">
        View Gallery
      </NuxtLink>
    </div>
    <div v-if="artistProfile && artistProfile.picture && artistProfile.picture.url" class="absolute right-8 top-0 bottom-0 my-auto aspect-square h-4/5">
      <div class="w-full h-full rounded-full bg-mimi-pink overflow-hidden">
        <img 
